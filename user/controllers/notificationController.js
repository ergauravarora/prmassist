...
const StoreToken = async (req, res, next) => {
  try {
    ...
    var { email, fcmToken } = req.body;
    const user = await userCrud.read(selector.byEmail(email));
    if (user.length > 0) {
      var fcm = user[0].fcmToken;
      if (fcm && fcm.length > 0) {
        var index = fcm.findIndex((x) => x == fcmToken);
      ...
      res.status(400).json({ user, msg: `No Data Found for email "${email}"` });
    }
  } catch (e) {
    console.log(e);
...
const SendNotification = async (req, res, next) => {
  try {
    //console.log(JSON.stringify(.id))
    var { email, notificationTitle, notificationText } = req.body;
    const user = await userCrud.read(selector.byEmail(email));
    if (user.length > 0) {
        ...
        .then((response) => {
          ...
          if (response.failureCount > 0) {
            response.responses.forEach((resp, idx) => {
              if (!resp.success) {
                failedTokens.push(
                  resp.error.message + "-" + registrationTokens[idx]
                );
              }
            });
          }
